...
- 在整个文本日志文件（或 Windows 事件日志）中搜索
  ...
  - 普通文本搜索始终只搜索原始日志内容；即使启用显示插件，也不会匹配插件处理后的字符
  - 普通文本搜索在未开启过滤时会直接复用底层流连续扫描原始文本，减少大文件和未命中搜索的额外开销
...
- 缺失配置提示：若插件目录中的 DLL 可发现，但启用时缺少必需配置文件，菜单会保留该插件并标记“缺少配置”，勾选时弹窗提示精确缺失原因。
